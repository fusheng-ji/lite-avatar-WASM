...
import numpy as np
# 兼容性处理：typeguard 的 check_argument_types 可能不存在
try:
...
class Speech2Text:
    ...
    def __call__(
        ...
        # Step A: log encoder/hidden_states shapes for ONNX export planning
        try:
            print("[DEBUG] enc shape:", tuple(enc.shape))
            if isinstance(hidden_states, (list, tuple)) and len(hidden_states) > 0:
                print("[DEBUG] num_layers:", len(hidden_states), "hid0:", tuple(hidden_states[0].shape))
            else:
                print("[DEBUG] hidden_states empty or not list/tuple")
        except Exception as dbg_e:
            print(f"[DEBUG] shape logging failed: {dbg_e}")
        # print(enc.size())
...
class ParaformerHiddenWrapper(torch.nn.Module):
    """
    Wrapper module for exporting Paraformer encoder hidden states to ONNX.

    Forward:
        inputs:
            feats: Tensor[B, T, D]   (frontend features; e.g. 80*m after LFR)
            feats_lengths: Tensor[B]
        outputs:
            hidden: Tensor[B, L, T, C]  # stacked hidden states
    """

    def __init__(self, asr_model: torch.nn.Module):
        super().__init__()
        self.asr_model = asr_model

    def forward(self, feats: torch.Tensor, feats_lengths: torch.Tensor):
        """
        Export-friendly forward that bypasses frontend FFT (wav_frontend).
        We directly run the encoder over precomputed features.
        """
        if getattr(self.asr_model.encoder, "interctc_use_conditioning", False):
            encoder_out, encoder_out_lens, _ = self.asr_model.encoder(feats, feats_lengths, ctc=self.asr_model.ctc)
        else:
            encoder_out, encoder_out_lens, _ = self.asr_model.encoder(feats, feats_lengths)

        if isinstance(encoder_out, tuple):
            enc = encoder_out[0]
            hidden_states = encoder_out[1]
        else:
            enc = encoder_out
            hidden_states = []

        # Stack hidden states: [L, B, T, C] -> [B, L, T, C]
        if isinstance(hidden_states, (list, tuple)) and len(hidden_states) > 0:
            stacked = torch.stack(hidden_states, dim=0)  # [L, B, T, C]
            hidden = stacked.permute(1, 0, 2, 3).contiguous()  # [B, L, T, C]
        else:
            # Fallback: only encoder output, treat as single layer
            hidden = enc.unsqueeze(1)  # [B, 1, T, C]

        return hidden


class ParaformerHiddenWrapperSingleInput(torch.nn.Module):
    """
    Wrapper module for exporting Paraformer encoder hidden states to ONNX with single input.
    This version only accepts 'feats' and automatically computes 'feats_lengths' from the input shape.
    ...
    def forward(self, feats: torch.Tensor):
        """
        Export-friendly forward that bypasses frontend FFT (wav_frontend).
        Automatically computes feats_lengths from feats shape.
        """
        # Compute feats_lengths from feats shape: [B, T, D] -> lengths = [T] for each batch
        # Use feats.shape[1] directly in multiplication to ensure ONNX treats this as dynamic
        # This prevents constant folding during ONNX export
        batch_size = feats.shape[0]
        # Multiply ones by shape[1] to ensure dynamic computation
        # ONNX will track feats.shape[1] as a dynamic dimension
        feats_lengths = (torch.ones(batch_size, dtype=torch.int32, device=feats.device) * 
                         feats.shape[1])
